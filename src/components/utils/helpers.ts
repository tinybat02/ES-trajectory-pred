...
export const processDataES = (data: SingleData[]) => {
  ...
  data.map(datum => {
    (perDeviceRoute[datum.trajectory_id] = perDeviceRoute[datum.trajectory_id] || []).push([
      datum.longitude,
      datum.latitude,
    ]);
    (perDeviceTime[datum.trajectory_id] = perDeviceTime[datum.trajectory_id] || []).push(datum.timestamp);
    (perDeviceFloor[datum.trajectory_id] = perDeviceFloor[datum.trajectory_id] || []).push(datum.level);
  });
